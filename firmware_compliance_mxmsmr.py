        ...
        def increase_limit(self):
            ...
            if self.current_limit < self.max_limit:
                ...
                if increase_amount > 1:
                    self.consecutive_successes -= 100
            self.last_adjustment_time = time.time()
...
def get_firmware_major_version(firmware_str):
    ...
    patterns = [
        ...
        r'[A-Z]+ (\d+\.\d+)(?:\.\d+)*',  # Format like "MX 18.211.5"
...
async def analyze_network_firmware(aiomeraki, networks, rate_limiter):
    ...
    firmware_stats = {
        'MX': {'Good': 0, 'Warning': 0, 'Critical': 0, 'Total': 0, 'Versions': defaultdict(int)},
        'MS': {'Good': 0, 'Warning': 0, 'Critical': 0, 'Total': 0, 'Versions': defaultdict(int)},
        'MR': {'Good': 0, 'Warning': 0, 'Critical': 0, 'Total': 0, 'Versions': defaultdict(int)}
    }
...
def export_firmware_to_csv(network_firmware_details, filename='mxmsmr_firmware_report.csv'):
...
async def generate(api_client, template_path, output_path, networks=None, inventory_devices=None, export_csv=False):
    """Generate the Firmware Compliance slide."""
    print(f"\n{GREEN}Generating Firmware Compliance slide (Slide 8)...{RESET}")
    ...
    try:
        ...
        firmware_stats = {
            'MX': {
                'Good': 72, 'Warning': 795, 'Critical': 10893, 'Total': 11760,
                'Versions': {
                    '18.211.5': 72,
                    '18.211.2': 794,
                    '18.211.4': 1,
            ...
            'MS': {
                'Good': 750, 'Warning': 0, 'Critical': 10980, 'Total': 11730,
                'Versions': {
                    '15.22.0': 82,
                    '15.23.0': 1,
                    '16.5.0': 70, 
            ...
            'MR': {
                'Good': 142, 'Warning': 11589, 'Critical': 1, 'Total': 11732,
                'Versions': {
                    '31.1.5': 142,
                    '30.7': 11542,
                    '30.7.1': 47,
        ...
        for i in range(50):
            ...
            network_firmware_details['MX'].append({
                'network_id': f'N_MX_{i}',
                'network_name': f'Test Network MX {i}',
    ...
    #print(f"{BLUE}Updating PowerPoint with firmware compliance data...{RESET}")
    ...
    try:
        ...
        for shape in slide.shapes:
            # Find title
            if hasattr(shape, "text_frame") and "Firmware Compliance" in shape.text_frame.text:
                title_shape = shape
                continue
        ...
        if not title_shape:
            ...
            title_p.text = "Firmware Compliance"
            title_p.font.size = Pt(44)
            title_p.font.bold = True
        
        # Add horizontal line across the full width of the slide, extending even further
        ...
        # Add "By Network" subtitle - MOVED UP BY 2 LINES
        ...
        product_positions = [product_cols['MX'], product_cols['MS'], product_cols['MR']]
        products = ['MX', 'MS', 'MR']
        ...
        divider2.line.color.rgb = RGBColor(200, 200, 200)
        ...
        print(f"{GREEN}Updated Firmware Compliance slide (Slide 8){RESET}")
    ...
    print(f"{PURPLE}Firmware Compliance slide generation completed in {ppt_time:.2f} seconds{RESET}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate firmware compliance slide and/or CSV export")
